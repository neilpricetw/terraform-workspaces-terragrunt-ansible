...
terraform {
  source = "${dirname(find_in_parent_folders("common_vars.hcl"))}/../modules//ec2_instance"
}
...
dependency "s3_bucket" {
    config_path = "${dirname(find_in_parent_folders("common_vars.hcl"))}/${local.account_name}/${local.region}/${local.env}/s3_bucket"
    skip_outputs = false
